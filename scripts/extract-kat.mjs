...
// -- ParaDrop packet (BIP-39 mnemonic drop, not PQHB-framed) --
// Mirrors paramant-relay sdk-js drop/pickup. Pure-Node: HKDF-SHA256 from the
// 16-byte entropy (salt "paramant-drop-v1") gives the AES key (info "aes-key")
// and the lookup id (SHA-256 of HKDF info "lookup-id"); AES-256-GCM with NO AAD;
// packet = nonce(12) || ct_len_be32 || ct. WebCrypto==pure-Node proven in
// scripts/derisk-paradrop.mjs. Fully deterministic given (entropy, nonce, pt) so
// the packet SHA-256 is a true anchor. The entropy<->mnemonic mapping is BIP-39
// (already covered by bip39.json); paramant-core self-checks the round-trip.
{
  const SALT = Buffer.from('paramant-drop-v1');
  const INFO_AES = Buffer.from('aes-key');
  const INFO_ID = Buffer.from('lookup-id');
  const hkdfExtract = (salt, ikm) =>
    createHmac('sha256', salt.length ? Buffer.from(salt) : Buffer.alloc(32)).update(Buffer.from(ikm)).digest();
  const hkdfExpand = (prk, info, len) => {
    const n = Math.ceil(len / 32);
  ...
  const LENGTHS = [0, 1, 15, 16, 32, 100, 255, 256, 1024, 4095, 4096, 60000, 250000, 1000000, 13];
  const vectors = LENGTHS.map((len, i) => {
    const id = `paradrop-${String(i).padStart(2, '0')}`;
    const entropy = bytesFrom(`paramant/paradrop/${i}/entropy`, 16);
    const nonce = bytesFrom(`paramant/paradrop/${i}/nonce`, 12);
    const plaintext = mkPt(len);
    const prk = hkdfExtract(SALT, entropy);
    const aesKey = hkdfExpand(prk, INFO_AES, 32);
    const idBytes = hkdfExpand(prk, INFO_ID, 32);
    const lookupId = createHash('sha256').update(Buffer.from(idBytes)).digest();
    const cipher = createCipheriv('aes-256-gcm', Buffer.from(aesKey), Buffer.from(nonce));
    const ct = Buffer.concat([cipher.update(Buffer.from(plaintext)), cipher.final(), cipher.getAuthTag()]);
    const packet = Buffer.concat([Buffer.from(nonce), u32be(ct.length), ct]);
    return {
      test_id: id,
      input: {
      ...
      expected: {
        aes_key_hex: hex(aesKey),
        lookup_id_hex: hex(lookupId),
        packet_len: packet.length,
        packet_sha256_hex: hex(createHash('sha256').update(packet).digest()),
      },
    };
  });

  write('envelope-paradrop', {
    primitive: 'envelope-paradrop',
    source: 'paramant-relay sdk-js drop/pickup: HKDF-SHA256(ikm=entropy, salt="paramant-drop-v1") -> aes-key + lookup-id; AES-256-GCM (no AAD); packet = nonce||ct_len_be32||ct; WebCrypto==pure-Node verified by scripts/derisk-paradrop.mjs.',
    note: 'derive(entropy) == (aes_key_hex, sha256 of lookup-id == lookup_id_hex); seal(entropy, nonce, plaintext) == packet of packet_len bytes with packet_sha256_hex; open recovers plaintext. Not PQHB-framed.',
    count: vectors.length,
    vectors,
  });
}
